...
## Relationship to the Orchestrator Skill

Use `claude-code-orchestrator` as the single source of truth for Hermes-level workflow and routing decisions.

- `claude-code-orchestrator` defines when Hermes should edit directly for small one-off fixes vs when Hermes should route to Claude Code for larger work.
- It also defines when to prefer persistent tmux sessions, when to split work across multiple Claude Code instances, when to write specs, and when to verify/push results.
- This `claude-code` skill is the lower-level operational reference for how to drive the Claude CLI once that routing decision has already been made.
- If guidance here conflicts with the orchestrator skill, follow `claude-code-orchestrator`.
...
### tmux + PTY Permission Dialogs (RECURRING PATTERN)
Claude Code often presents confirmation dialogs for file system operations or shell commands. When orchestrating via tmux, look for the following TUI prompts in `capture-pane`:
- `Do you want to proceed? ❯ 1. Yes`
- `The directory ... contains files that could conflict. ❯ 1. No, exit`

**Crucial Logic for Conflicts/Initialization:**
If Claude encounters conflicts (e.g., `create-next-app` in a non-empty directory), it may try to pivot to a `/tmp` directory. You MUST follow its lead:
1. `tmux capture-pane` to identify the block.
2. `tmux send-keys -t <session> Enter` to accept the default "Yes" or navigate with `Down` for alternatives.
3. If it pivots to `/tmp`, remind it to move the files back to the target project directory.

### Interactive "Wrangling"
When Claude shows `* Wrangling...` or `✻ Wrangling...`, it is performing deep reasoning or parallel file operations. Do not interrupt with new instructions during this phase; instead, poll `capture-pane` every 30-60 seconds to detect when the `❯` prompt returns or if a new permission dialog appears.
...
## Multi-Tasking during Long Generations (Wait Monitoring)

When Claude is "Wrangling" or performing massive file generations (which can take 5-10+ minutes for large legacy conversions):
- **Don't sit idle:** Use the time to perform parallel research, asset collection (browser tools), or file system preparation in the main Hermes session.
- **Background Context:** If the user provides a major project pivot (e.g., "this is now a shop page") during a generation, send a `tmux send-keys` message immediately. Claude will incorporate the new context into its next set of actions.
- **Avoid Interruption:** Do not use `Ctrl+C` or `esc` on a wrangling task unless it's clearly stuck (no token movement for > 2 mins); large-scale refactors often require significant internal reasoning time.

## PTY Syncing Pitfalls

- **Standard PTY vs tmux:** While `terminal(pty=true)` works for simple interactions, `tmux` is mandatory for **persistent** coding sessions where you need to check status, send background updates, and capture multi-pane activity.
- **Ghost Input:** If a `tmux send-keys` command doesn't seem to trigger a response, check if Claude is in a block (like a "Do you want to proceed?" prompt). Use `capture-pane` frequently to stay synced with the TUI state.
...
## Orchestrating Large-Scale Rebuilds (Experiential Findings)

When using Claude Code to rebuild a legacy monolith (e.g., a 2,000+ line `index.html`) into a modern framework like Next.js:

1. **Expect "Wrangling" Delays**: For massive files, Claude will spend significant time (5-10+ mins) in a "Wrangling" state. This is deep reasoning/mapping. Do not interrupt; monitor via `capture-pane`.
2. **Handle Batch Approvals**: Claude will often batch create 10+ files. If using `interactive` mode, you must handle the overwrite/create confirmations. Using `tmux send-keys -t <session> "2" Enter` (Yes, allow all edits) is critical to prevent the process from hanging on individual file approvals.
3. **Strategic Pivot Pattern**: 
   - Phase 1: Structural Rebuild (Technical completion of components).
   - Phase 2: Content Pivot (Swapping branding/assets).
   - *Strategy*: Keep these phases distinct. Let Claude finish the technical skeleton before introducing high-level strategy changes to avoid context confusion.
4. **Git Checkpoints**: Force git commits (`tmux send-keys ... "git add . && git commit -m '...' "`) after major component batches. Large rebuilds are high-risk; having "save points" allows for modular pivots without losing structural work.
5. **Parallel Asset Scouting**: While Claude is "Wrangling," use the Hermes `browser` tools to scout the target site, download assets, and notate precise design instructions in a `Strategy/` folder. This keeps the architect (Hermes) and the builder (Claude) working in parallel.



1. **Interactive mode REQUIRES tmux** — Claude Code is a full TUI app. Using `pty=true` alone in Hermes terminal works but tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration.
2. **`--dangerously-skip-permissions` dialog defaults to "No, exit"** — you must send Down then Enter to accept. Print mode (`-p`) skips this entirely.
3. **Root/sudo restriction is real** — Claude Code may refuse `--permission-mode bypassPermissions` / dangerous-skip-permissions when running as root. On root-owned environments, prefer `claude -p` with explicit `--allowedTools` for autonomous runs, including parallel background jobs.
...
3. **If running as root blocks bypass-permissions modes, switch to print mode + explicit `--allowedTools`** instead of stalling on interactive permission flows
...
9. **Push inspectable Claude work** — after each accepted fix or iteration, verify, commit, and push so Lionel can inspect the live site
10. **Use separate routes for experiments** — if Claude is testing an alternate concept or risky UI change, create a new page/route for A/B testing instead of overwriting the main page in place
11. **Report results to user** — after completion, summarize what Claude did and what changed
12. **Don't kill slow sessions** — Claude may be doing multi-step work; check progress instead
13. **Use `--allowedTools`** — restrict capabilities to what the task actually needs
